...
#   ~/.claude/plugins/known_marketplaces.json → config/.claude/plugins/known_marketplaces.json
#   ~/.claude/plugins/installed_plugins.json → config/.claude/plugins/installed_plugins.json
#
# What does NOT get synced:
#   .credentials.json — secrets, never committed
#   plugins/cache/, plugins/marketplaces/ — large binary cache, re-fetched on VPS
#
# Usage:
...
sync_dir() {
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        rm -rf "$dst"
        # -L dereferences symlinks so VPS gets real files, not broken links
...
# Sync a single file with diff preview
sync_file() {
    local name="$1" src="$2" dst="$3"
    [ -f "$src" ] || return
    if [ "$DRY_RUN" = true ]; then
        if ! diff -q "$src" "$dst" &>/dev/null; then
            echo -e "  ${CYAN}$name:${NC} changed"
        else
            echo -e "  ${GREEN}$name:${NC} no changes"
        fi
    else
...
sync_file "keybindings.json"    "$LOCAL_DIR/keybindings.json"           "$CONFIG_DIR/keybindings.json"
sync_file "settings.json"       "$LOCAL_DIR/settings.json"              "$CONFIG_DIR/settings.json"
sync_file "statusline-command.sh" "$LOCAL_DIR/statusline-command.sh"    "$CONFIG_DIR/statusline-command.sh"
sync_file "plugins/known_marketplaces.json" "$LOCAL_DIR/plugins/known_marketplaces.json" "$CONFIG_DIR/plugins/known_marketplaces.json"
sync_file "plugins/installed_plugins.json"  "$LOCAL_DIR/plugins/installed_plugins.json"  "$CONFIG_DIR/plugins/installed_plugins.json"
