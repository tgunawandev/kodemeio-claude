...
name: waha-admin
description: >
  WAHA (WhatsApp HTTP API) administration for kodemeio infrastructure.
  Supports multiple instances via profiles. Covers WhatsApp session management,
  message sending, webhook configuration, bridge integration (Chatwoot/Odoo),
  anti-detection monitoring, health checks, and dashboard overview.
  Use when working with kctl-waha CLI or managing any WAHA instance.
version: 1.0.0
allowed-tools:
  - Bash
  - Read
