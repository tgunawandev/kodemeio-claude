...
name: gatus-admin
description: >
  Gatus health monitoring administration for kodemeio infrastructure.
  Supports multiple Gatus instances via profiles (gatus.kodeme.io, etc.).
  Covers endpoint monitoring, check results, uptime/response-time queries,
  alert channel testing, auto-discovery sidecar management, health checks,
  and dashboard overview. Use when working with kctl-gatus CLI or managing
  any Gatus instance.
version: 1.0.0
allowed-tools:
  - Bash
